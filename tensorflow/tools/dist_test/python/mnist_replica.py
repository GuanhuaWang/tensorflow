...
#replica sync. initialization
    if FLAGS.sync_replicas:
      ...
      opt = tf.train.SyncReplicasOptimizer(opt,relicas_to_aggregate=replicas_to_aggregate,total_num_replicas=num_workers,name="mnist_sync_replicas")
#replica sync. initialization

#train optimization function
...
#replica sync.
    ...
    if is_chief:
      print("Worker %d: Initializing WGH session..." % FLAGS.task_index)
    else:
      print("Worker %d: Waiting for WGH session to be initialized..." % FLAGS.task_index)

    if FLAGS.existing_servers:
      server_grpc_url = "grpc://" + worker_spec[FLAGS.task_index]
      print("Using existing server at: %s" % server_grpc_url)

      sess = sv.prepare_or_wait_for_session(server_url,config=sess_config)

    else:
      sess = sv.prepare_or_wait_for_session(server.target,config=sess_config)

    print("Worker %d : Session 2WGH initialization complete." % FLAGS.task_index)
    ...
    print("After %d training steps, validation cross entropy= %g" % (FLAGS.train_steps,val_xent))
